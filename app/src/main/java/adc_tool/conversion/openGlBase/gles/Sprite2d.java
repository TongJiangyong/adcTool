...
public class Sprite2d {
    ...
    /**
     * Draws the rectangle with the supplied program and projection matrix.
     * //这里的projectionMatrix有宽高信息，因此能够整合和平移对象....将其float化......
     */
    public void draw(Texture2dProgram program, float[] projectionMatrix) {
        // Compute model/view/projection matrix.
        //projectionMatrix这里加入了projectionMatrix的判断
        //如果视图上只需要绘制其中的一部分，则一定需要这个projectionMatrix，否则调用单位阵就好
        // result = lhs x rhs  即结果是投影矩阵左乘模型视图矩阵，这里只是模型视图矩阵 然后这个其实改变的都是模型视图矩阵
